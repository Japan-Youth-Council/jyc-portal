...
export default function Header() {
  ...
  // ▼ 追加：データベースから取得したプロフィール名を保持するステート
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (session?.user) {
        ...
        // ▼ 追加：ユーザーが存在する場合、profilesテーブルから名前を取得
  ...
  // ▼ 追加：表示する名前の決定（プロフィール名があればそれを、なければGoogleの名前を、それもなければ「名称未設定」）
  const displayName = profileName || user.user_metadata?.full_name || '名称未設定';
        ...
        <div className="flex items-center space-x-3">
          ...
          <div className="hidden md:flex items-center space-x-4">
            <div className="relative" ref={userMenuRef}>
              ...
              >
                {/* ▼ 修正：displayName を使用 */}
                <span className="text-sm font-bold text-gray-700 truncate max-w-[150px]">{displayName}</span>
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white border-b shadow-lg flex flex-col p-5 space-y-6 animate-in slide-in-from-top-2">
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              {/* ▼ 修正：displayName を使用 */}
              <span className="text-base font-bold text-gray-800 truncate">{displayName}</span>
